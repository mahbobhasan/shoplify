  ...
  .then(orderItems => {
    ...
    orderItems.forEach(item => {
      const row = `
        <tr>
          ...
          <td>
            <p class="mb-0 py-4">${(parseFloat(item.product_price) * item.quantity).toFixed(2)} Taka</p>
          </td>
          <td class="py-4">
            <button class="btn btn-md rounded-circle bg-light border delete-btn" data-id="${item.product}">
    ...
    tbody.addEventListener("click",async function (e) {
      ...
      if (e.target.closest(".delete-btn")) {
        ...
        try {
          const response = await fetch(`http://127.0.0.1:8000/orders/order-items/${orderId}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
                "Authorization":localStorage.getItem("Authorization")
